...
public class CropBorder extends View {
	...
	protected BorderDrawable mBorderDrawable;// 浮层
	...
	public CropBorder(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public CropBorder(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setDrawable(Drawable mDrawable,Bitmap borderBitmap, int cropWidth, int cropHeight) {
		this.mDrawable = mDrawable;
		this.cropWidth = cropWidth;
		this.cropHeight = cropHeight;
		this.isFrist = true;
		mBorderDrawable = new BorderDrawable(mContext, borderBitmap);
		invalidate();
	}
	...
	protected void configureBounds() {
		...
		mBorderDrawable.setBounds(mDrawableFloat);
	}
	...
	public Bitmap buildConcatImage (int targetWidth,Bitmap originBorder) {
		
		int tmpWidth = targetWidth + 100;
		int tmpHeight = (int) (targetWidth * (Float.valueOf(getHeight()) / getWidth()));
		Bitmap tmpBitmap = Bitmap.createBitmap(tmpWidth, tmpHeight, Config.ARGB_8888);
		
		Canvas resultCanvas = new Canvas(tmpBitmap);
		
		//1,底图 所见 -> 目标 坐标点转换
		float resultScale = Float.valueOf(targetWidth) / mDrawableFloat.width();
		Log.d("crop", "resultScale-->" + resultScale);
		Rect targetRect = new Rect();
		targetRect.left = (int) (mDrawable.getBounds().left * resultScale);
		targetRect.top = (int) (mDrawable.getBounds().top * resultScale);
		targetRect.right = (int) (mDrawable.getBounds().right * resultScale);
		targetRect.bottom = (int) (mDrawable.getBounds().bottom * resultScale);
		
		mDrawable.setBounds(targetRect);
		mDrawable.draw(resultCanvas);
		
		//2,画 边框
		Rect borderRect = new Rect();
		borderRect.left = (int) (mDrawableFloat.left * resultScale);
		borderRect.top = (int) (mDrawableFloat.top * resultScale);
		borderRect.right = (int) (mDrawableFloat.right * resultScale);
		borderRect.bottom = (int) (mDrawableFloat.bottom * resultScale);
		
		BitmapDrawable tmpBorder = new BitmapDrawable(originBorder);
		tmpBorder.setBounds(borderRect);
		tmpBorder.draw(resultCanvas);
		
		//回收
		bitmapRelase(originBorder);
		
		//最终合成图
		Bitmap resultBitmap = Bitmap.createBitmap(tmpBitmap, borderRect.left, borderRect.top, borderRect.width(), borderRect.height());
		bitmapRelase(tmpBitmap);
		
		return resultBitmap;
	}
	
	private void bitmapRelase(Bitmap b){
		b.recycle();
		b=null;
	}
